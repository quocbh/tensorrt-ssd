...
nvinfer1::IPlugin* PluginFactory::createPlugin(const char* layerName, const nvinfer1::Weights* weights, int nbWeights)
{
    ...
    if (!strcmp(layerName, "conv4_3_norm"))
    {
        ...
        mNormalizeLayer = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>
                (createSSDNormalizePlugin(weights, false, false, 0.0001), nvPluginDeleter);
    ...
    else if (!strcmp(layerName, "mbox_conf_reshape_perm"))
    {
        assert(mConv9_2_mbox_loc_perm_layer.get() == nullptr);
        std::cout <<  " Error"<<std::endl;
        mConv9_2_mbox_loc_perm_layer = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>
                (createSSDPermutePlugin({{0, 2, 1, 3}}), nvPluginDeleter);
        return mBox_conf_reshape_perm_layer.get();
    }
    ...
    else if (!strcmp(layerName, "pool6_mbox_priorbox"))
    {
        ...
        float min_size = 264.0, max_size = 315.0, aspect_ratio[2] = {1.0, 2.0};
        mPool6_mbox_priorbox_layer = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>
                (createSSDPriorBoxPlugin({&min_size, &max_size, aspect_ratio, 1, 1, 2, true, false, {0.1, 0.1, 0.2, 0.2}, 0, 0, 300.0, 300.0, 0.5}), nvPluginDeleter);
    ...
    else if (!strcmp(layerName, "mbox_conf_reshape"))
    {
        ...
        mMbox_conf_reshape = std::unique_ptr<Reshape<NUM_CLASSES>>(new Reshape<NUM_CLASSES>());
    ...
    else if (!strcmp(layerName, "detection_out"))
    {
        ...
        mDetection_out = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>
                (createSSDDetectionOutputPlugin({true, false, 0, NUM_CLASSES, 400, 200, 0.2, 0.45, CodeType_t::CENTER_SIZE}), nvPluginDeleter);
...
IPlugin* PluginFactory::createPlugin(const char* layerName, const void* serialData, size_t serialLength)
{
    ...
    {
        assert(mConv9_2_mbox_loc_perm_layer.get() == nullptr);
        mBox_conf_reshape_perm_layer = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>(createSSDPermutePlugin(serialData, serialLength), nvPluginDeleter);
        return mBox_conf_reshape_perm_layer.get();
    }
    else if (!strcmp( layerName, "conv4_3_norm_mbox_priorbox"))
    {
    ...
    {
        assert(mPool6_mbox_priorbox_layer.get() == nullptr);
        mPool6_mbox_priorbox_layer = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>(createSSDPriorBoxPlugin(serialData, serialLength), nvPluginDeleter);
        return mPool6_mbox_priorbox_layer.get();
    }
    ...
    else if (!strcmp(layerName, "mbox_conf_reshape"))
    {
        ...
        mMbox_conf_reshape = std::unique_ptr<Reshape<NUM_CLASSES>>(new Reshape<NUM_CLASSES>(serialData, serialLength));
        return mMbox_conf_reshape.get();
    }
    //softmax
    else if (!strcmp(layerName, "mbox_conf_softmax"))
...
bool PluginFactory::isPlugin(const char* name)
{
    return (!strcmp(name, "conv4_3_norm")
            ...
            || !strcmp(name, "mbox_conf_reshape_perm")
...
//    mBox_conf_reshape_perm_layer.release();
//    mBox_conf_reshape_perm_layer = nullptr;
...
//    mPool6_mbox_priorbox_layer.release();
//    mPool6_mbox_priorbox_layer = nullptr;
